...
void call_ASPS(model& our_model)
{
	...
	auto start_s_s = chrono::high_resolution_clock::now();
	for (int wide_e_index = 0; wide_e_index < our_model.num_edges; wide_e_index++)
	{
		if(wide_e_index % 50==0){
			int currentProgress = (double)(wide_e_index + 1)/our_model.num_edges * 100;
			auto end_s_s = chrono::high_resolution_clock::now();
			double time = (chrono::duration_cast<chrono::nanoseconds>(end_s_s - start_s_s).count()) / 1000000000.0;
			cout <<"finished edge_"<< wide_e_index<<" Progress: " << currentProgress << "%  (Duration:" << time <<"s) "<<endl;
			start_s_s=chrono::high_resolution_clock::now();
		}
...
void call_SAM(model& our_model){
	...
	auto start_s_s = chrono::high_resolution_clock::now();
	// int progressInterval = our_model.num_edges / 100.0;
	for (int wide_e_index = 0; wide_e_index < our_model.num_edges; wide_e_index++)
	{
		if(wide_e_index % 50==0){
			int currentProgress = (double)(wide_e_index + 1)/our_model.num_edges * 100;
		...
		for (int l = 0; l < our_model.L; l++)
		{
			...
			//sampling edge
			sort_selection(our_model,wide_e_index);
			...
			for (int e = 0; e < num_edges_two_sets; e++)
			{
				...
				{	
					...
					if (our_model.sp_node_vec_first[node_u].cur_sp_value > our_model.tau_vec[our_model.D-1] &&
					our_model.sp_node_vec_first[node_v].cur_sp_value > our_model.tau_vec[our_model.D-1] &&
					our_model.sp_node_vec_second[node_u].cur_sp_value > our_model.tau_vec[our_model.D-1] &&
